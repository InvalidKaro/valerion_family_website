  ...
  return (
    <main>
      <div className={headerStyles.boxHeader}>
        <header className={`${headerStyles.header} ${isScrolled ? headerStyles.hidden : ''}`}>
          <a href="/"><img src={logo} alt="logo" className={headerStyles.header__logo} /></a>
          <nav>
            <ul className={headerStyles.header__menu}>
              <li className={headerStyles.header__link}><a href="History">Family Tree</a></li>
              <li className={headerStyles.header__link}><a href="Supporters">Supporters</a></li>
              <li className={headerStyles.header__link}>
                <div className={headerStyles.trapezoid}>
                  ...
                  <a href="Shop">
                    <text x="150" y="40" dominant-baseline="middle" text-anchor="middle" fill="white"  class={headerStyles.market_tex}>
                      Market
                    </text>
                  </a>
                </svg>
              ...
              <li className={headerStyles.header__link}><a href="Family">FAQ's</a></li>
              <li className={headerStyles.header__link}><a href="Help">Need Help?</a></li>
            </ul>
          </nav>
          <div className={headerStyles.header__rsection}>
          <button className={headerStyles.buy_button} type="button" onClick={handleBuyClick}>BUY</button>
            <div className={headerStyles.header__user__section}>
              <a href="#" onClick={handleUserIconClick} className={headerStyles.user_icon}>
              <img
        ...
        {isLoggedIn && dropdownVisible && (
              <div className={headerStyles.dropdown_menu}>
                  <div className={headerStyles.dropdown}>
                    <button className={headerStyles.dropdown_button} onClick={() => {navigate(`/user/${user.username}`); setDropdownVisible(false)}}>Profile</button>
                    <button className={headerStyles.dropdown_button} onClick={() => {navigate('/Settings'); setDropdownVisible(false)}}>Settings</button>
                    <button className={headerStyles.dropdown_button} onClick={() => { handleLogout(); setDropdownVisible(false); }}>
  Logout
</button>                  </div>  
              </div>
            )}
